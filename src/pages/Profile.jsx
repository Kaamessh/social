...
const Profile = () => {
    ...
    const handleChatMediaUpload = async (e) => {
        try {
            ...
            const filePath = `chats/${user.id}_${Date.now()}.${file.name.split('.').pop()}`
            ...
            alert('MEDIA SEND FAIL: ' + err.message)
        } finally {
    ...
    if (view === 'messaging') {
        return (
            <div className="container" style={{ height: 'calc(100vh - 120px)', display: 'flex', flexDirection: 'column' }}>
                ...
                <form onSubmit={sendMessage} style={{ display: 'flex', gap: '1rem', padding: '1rem 0', alignItems: 'center' }}>
                    <label style={{ cursor: 'pointer', padding: '10px', background: 'var(--surface)', border: '1px solid var(--border)', fontWeight: 900 }}>
                        {isMediaLoading ? '⏳' : '📷'}
                        <input type="file" hidden accept="image/*" onChange={handleChatMediaUpload} disabled={isMediaLoading} />
                    </label>
